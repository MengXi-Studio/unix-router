...
| `SETUP_ERROR` | `128` | 路由安装环境错误（uni-app x 扩展；预留，当前实现未抛出） |
| `PLUGIN_REQUIRED` | `256` | 使用插件能力但未注册对应插件（如未注册 `ParamsPlugin` 却使用 `params`、未注册 `EventsPlugin` 却使用 `events`）（uni-app x 扩展） |

::: tip 判断方式
配合 `isNavigationFailure(error, type?)` 收窄判断（`type` 为单个 `RouterErrorCode`，缺省 `null` 时仅判断是否为 `NavigationFailure`），或直接比较（`failure.code === RouterErrorCode.ROUTE_NOT_FOUND`）。`NavigationFailure` 的 `to` / `from` 字段可进一步定位失败上下文。
:::

## 相关 API

- [isNavigationFailure 与错误处理指南](../guide/error-handling)
